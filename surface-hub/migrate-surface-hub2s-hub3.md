...
- [Manually install Migration Launcher app on Surface Hub 2S](#). Designed for migrating individual Surface Hub 2S devices.
- [Remotely install Migration Launcher app via Intune.](#) Designed for migrating multiple select Surface Hub 2S devices remotely.
...
4. Once you see it in the app list, the migration process starts automatically upon reboot into the Windows 10 Team OS, which can be initiated either manually as soon as the app is validated to be installed or automatically during the nightly maintenance window.

> [!TIP]
> After this initial reboot, do not interfere with the system; even though it looks like nothing is happening and you still see the old OS, the migration proceeds independently after completing all necessary backend processes (including downloading and unzipping the new OS image).

5. Typical migration time is around **90** minutes from reboot after installing the Migration Launcher app, but the length can vary depending on many characteristics, including customer network bandwidth and related factors. 
...
1. Ensure your Surface Hub 2S devices are **enrolled in Intune**.
2. Add the Surface Hub 2S devices targeted for migration to a **Device group**.
3. Download the **Migration Launcher app** from the **Microsoft Store for Business**.
4. Deploy the Migration Launcher app via **Intune**, adding the Device group from Step 2.
...
TIP: This is a good opportunity to check that the OS version for the target devices meets the prerequisites listed above, specifically OS version 19045.3758.
...
3. If you don't specifically have Microsoft Intune available as a mobile device management tool, choose **Add management tool** to add Microsoft Intune. If you don't have Microsoft Intune activated as your mobile device management tool, select **Microsoft Intune** >**Activate**. 
...
#### Configure the Microsoft Store for Business website to display offline apps

1. Sign in to the [Microsoft Store for Business](https://businessstore.microsoft.com/).
2. Select **Manage** >**Shop** and under Shopping experience, ensure that **Show offline apps** is enabled.

#### Get the offline-licensed app package for the Migration Launcher
...
- If you receive an error indicating the downloadable files are unsafe, click **Keep** and **Keep anyway**.
7. You should now have four files (one MSIXBUNDLE File and three APPX Files) downloaded to your local computer and ready for the next step.
...
- Once the app is deployed, the next reboot initiates migration, which - in this remote deployment scenario - is likely to be the automatic reboot as part of the nightly maintenance window. Similar to the manual app deployment scenario, the migration process typically takes about 90 minutes after this reboot (but many factors can impact migration time, as explained earlier, with a test example exceeding 5 hours on a slower network). Customers must test and validate sequences relative to their unique infrastructure and management approaches.
...
- Verify that Surface Hub 2S is fully updated and otherwise meets the prerequisites above. To learn more, see [Troubleshoot Surface Hub not Updating - Surface Hub | Microsoft Learn](https://teams.microsoft.com/l/message/19:meeting_ZjJkY2Y4MzQtYjU2Ny00OTNiLTgxZGEtNjViY2UzNTQwMDg0@thread.v2/1701731777431?context=%7B%22contextType%22%3A%22chat%22%7D)
- Ensure the Migration Launcher app has been installed on target Surface Hub 2S devices.
- Ensure the system has rebooted after installing the Migration Launcher app.
- If the above has been verified, and migration still has not occurred after at least a full day, consult system event logs and check for possible errors, including a potential issue related to available storage space. Contact Support if needed.
- For remote deployment scenarios using Intune:

- If the Migration Launcher app fails to deploy, check the error messages in the Intune console for clues.
- Verify that the Surface Hub 2S devices are correctly enrolled and online.
- Consider verifying whether another Store for Business offline-licensed application can be remotely deployed to the Surface Hub 2S device(s) in question.

## Post-migration configuration

In addition to following the same first-run experience steps as Surface Hub 3 (starting with the Windows OOBE), consider leveraging these additional resources on system configuration for your newly migrated Surface Hub 2S device(s) now running the Microsoft Teams Rooms on Windows experience. To learn more, see [First -time setup for Surface Hub 3](https://learn.microsoft.com/surface-hub/first-run-program-surface-hub-3).
